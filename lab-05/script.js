const data = [
 ...
 function populateTeamFilter(){
    ...
    const teams = [...new Set(data.map(p => getAbbr(p.team)))].sort();
    teams.forEach(abbr => {
...
function renderRows(players){
    const today = document.getElementById("player-rows");
    today.innerHTML = "";
    players.forEach(player => {
        const row = document.createElement("tr");
        row.innerHTML = `
        <td>${player.name}</td>
        <td>${getAbbr(player.team)}</td>
        <td>${player.points}</td>
        <td>${player.rebounds}</td>
        <td>${player.assists}</td>
        `;

        row.addEventListener("mouseover",() => row.style.backgroundColor = "#fffdd0c5" );
...
function getFilteredData() {
    ...
    return data.filter(player => {
        const matchsName = player.name.toLowerCase().includes(query);
        const matchesTeam = team === "all" || getAbbr(player.team) === team;
        return matchsName && matchesTeam;
    });
}

document.getElementById("search").addEventListener("input", () => renderRows(getFilteredData()));
document.getElementById("team-filter").addEventListener("change", () => renderRows(getFilteredData()));
